...
create_boxplot <- function(data, y_var, title) {
  ggplot(data, aes(x = Classification, y = !!sym(y_var), fill = zone)) +
    geom_boxplot(color = "black", outlier.shape = NA, alpha = 0.7) +
    labs(y = title) +
    theme_minimal() +
    scale_y_continuous(limits = c(0, 13)) +
...
plot1 <- ggplot() +
  ...
  coord_fixed() +  # Ensure square layout
...
# Arrange plots side by side and set width
...
#####Are zone diversities different? YES according to PERMANOVA
diversity.df<-na.omit(diversity.df)
...
invertbiomass <- read.csv(file = "updated_BiomassR.csv", header = T)
...
invertbiomass$count <- rep(1,24640)
